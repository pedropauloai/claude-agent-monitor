    ...
    async function load() {
      try {
        ...
        opts.sort((a, b) => {
          ...
          if (a.status !== 'active' && b.status === 'active') return 1;
          return 0; // preserve API order (already by startedAt DESC)
        });
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {isOpen && options.length > 0 && (
        <div className="absolute top-full mt-1 right-0 z-[100] w-72 max-h-64 overflow-y-auto rounded-lg border border-cam-border/50 bg-cam-bg shadow-xl">
          {(() => {
              ...
              <>
                ...
                {activeOpts.length > 0 && inactiveOpts.length > 0 && (
                  <div className="px-3 py-1.5 border-t border-cam-border/30">
                    <span className="text-[9px] uppercase tracking-wider text-cam-text-muted">Ended</span>
                  </div>
                )}
                {inactiveOpts.map((opt) => {
                  ...
                  return (
                    <button
                      key={opt.id}
                      onClick={() => selectSession(opt)}
                      className={`w-full px-3 py-2 flex items-center gap-2 text-left text-xs hover:bg-cam-surface/60 transition-colors opacity-60 ${
                        isActive
                          ? "bg-cam-surface/40 border-l-2 border-cam-accent opacity-100"
                          : ""
                      }`}
                    >
                      <StatusDot status={opt.status} />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="terminal-dim ml-1">{opt.name ? opt.label : opt.sublabel}</span>
                      </div>
